...
async function router(req, res) {
  ...
  // ── HEALTH CHECK (Railway uses this to confirm app is up) ──
  ...
  if (method === 'POST' && pathname === '/api/auth/register') {
    try {
      ...
      const user = db.prepare(
        'SELECT id,name,email,role,branch,year FROM users WHERE id=?'
  ...
  if (!payload || !payload.id || !payload.role) return unauth(res);
  ...
  if (method === 'POST' && pathname === '/api/events') {
    ...
    const r = db.prepare(
      'INSERT INTO events(title,description,category,date,time,venue,capacity,branch,organizer_id) VALUES(?,?,?,?,?,?,?,?,?)'
  ...
  if (method === 'GET' && pathname === '/api/recommendations') {
    try {
      ...
      const history = db.prepare(`
        SELECT e.category FROM registrations r
        JOIN events e ON r.event_id=e.id
      ...
      if (topCats.length === 0) {
        recs = db.prepare(
          `SELECT * FROM events WHERE id NOT IN (${placeholders}) ORDER BY date LIMIT 5`
        ).all(...excludeIds);
      } else {
        const catPlaceholders = topCats.map(() => '?').join(',');
        recs = db.prepare(`
          SELECT * FROM events
          WHERE id NOT IN (${placeholders})
          AND (category IN (${catPlaceholders}) OR branch=? OR branch='All')
  ...
  if (method === 'GET' && pathname === '/api/admin/users') {
    if (userRole !== 'admin') return err(res, 'Forbidden', 403);
    return ok(res, db.prepare('SELECT id,name,email,role,branch,year,created_at FROM users').all());
  }
